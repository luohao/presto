...
import com.facebook.presto.druid.zip.ZipFileEntry.Compression;
...
class CentralDirectoryFileHeader
{
    ...
    static long read(ZipFileData fileData, IndexSource indexSource, long fileOffset, Charset charset)
            throws ZipException
    {
        long position = fileOffset;

        byte[] fixedSizeData = new byte[FIXED_DATA_SIZE];
        indexSource.readFully(position, fixedSizeData);
        position += fixedSizeData.length;
        if (!ZipUtil.arrayStartsWith(fixedSizeData, ZipUtil.intToLittleEndian(SIGNATURE))) {
            throw new PrestoException(DRUID_ZIP_INDEX_FILE_ERROR, String.format("Malformed Central Directory File Header; does not start with %08x", SIGNATURE));
        }
        ...
        if (name.length > 0) {
            indexSource.readFully(position, name);
            position += name.length;
        }
        if (extraField.length > 0) {
            indexSource.readFully(position, extraField);
            position += extraField.length;
        }
        if (comment.length > 0) {
            indexSource.readFully(position, comment);
            position += extraField.length;
        }
