...
import com.twitter.presto.druid.zip.CentralDirectoryFileHeader;
import com.twitter.presto.druid.zip.EndOfCentralDirectoryRecord;
import com.twitter.presto.druid.zip.LocalFileHeader;
import com.twitter.presto.druid.zip.Zip64EndOfCentralDirectory;
import com.twitter.presto.druid.zip.Zip64EndOfCentralDirectoryLocator;
import com.twitter.presto.druid.zip.ZipFileData;
import com.twitter.presto.druid.zip.ZipFileEntry;
import com.twitter.presto.druid.zip.ZipUtil;
        ...
        implements IndexFileSource, Closeable, AutoCloseable
{
            ...
            throws IOException
    {
        ...
        if (readLength < EndOfCentralDirectoryRecord.FIXED_DATA_SIZE) {
            throw new PrestoException(DRUID_ZIP_INDEX_FILE_ERROR, format("Zip file '%s' is malformed. It does not contain an end of central directory record.", dataInputSource.getId()));
        }
